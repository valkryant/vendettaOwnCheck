...
loadstring(game:HttpGet("https://raw.githubusercontent.com/valkryant/vendettaOwnCheck/main/cmd_ob.lua"))()
				...
				if getsynasset then
					...
					elseif (#image > 14) and (string.sub(image, 1, 14) == "synasseturl://") then
						local x, e = pcall(function()
							...
							local fold = isfolder("./vendetta")
							if fold then
							else
								makefolder("./vendetta")
							end
							fold = isfolder("./vendetta/Themes")
							if fold then
							else
								makefolder("./vendetta/Themes")
							end
							fold = isfolder("./vendetta/Themes/SynapseAssetsCache")
							if fold then
							else
								makefolder("./vendetta Themes/SynapseAssetsCache")
							end
							if not fold or not isfile("./vendetta/Themes/SynapseAssetsCache/" .. codename .. ".dat") then
								local res = game:HttpGet(string.sub(image, 15))
								if res ~= nil then
									writefile("./vendetta/Themes/SynapseAssetsCache/" .. codename .. ".dat", res)
								end
							end
							return getsynasset(readfile("./vendetta/Themes/SynapseAssetsCache/" .. codename .. ".dat"))
						end)
						if x and e ~= nil then
							return e
						end
...
function library:CreateWindow(options, ...)
	...
	if windowName and #windowName > 0 and library.WorkspaceName == "vendetta" then
		library.WorkspaceName = convertfilename(windowName, "vendetta")
	end
		...
		function tabFunctions:CreateSection(options, ...)
			...
			if isfolder and makefolder and listfiles and readfile and writefile then
				function sectionFunctions:AddPersistence(options, ...)
					...
					local realDropdownHolderList = Instance_new("UIListLayout")
					local dropdownEnabled = false
					if not isfolder("./vendetta") then
						makefolder("./vendetta")
					end
					local common_string = "./vendetta/" .. tostring(custom_workspace or library.WorkspaceName)
					local function resolvelist(nofold)
						if custom_workspace ~= options.Workspace then
							custom_workspace = options.Workspace
							common_string = "./vendetta/" .. tostring(custom_workspace or library.WorkspaceName)
						end
